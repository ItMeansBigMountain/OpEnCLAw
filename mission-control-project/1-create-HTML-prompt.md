...
About Me:
- Name: Affan
- Business/Role: Oyama Productions - CEO
- Main Goal: Build a fully autonomous, agent-driven revenue system that uses a coordinated team of AI agents to continuously identify high-value opportunities, produce and distribute optimized content at scale, capture and convert attention into owned assets such as email lists and traffic, and systematically reinvest performance insights into increasingly effective strategies—creating compounding, self-improving revenue streams with minimal human intervention.
- Key Metric: [Positive Revenue]

Technical Requirements:
- Single self-contained HTML file (inline CSS + JS, no external dependencies except Google Fonts Inter)
- Dark theme with glassmorphism: primary bg #050508, cards with backdrop-filter blur, subtle borders rgba(255,255,255,0.06) and some military camos placed with good taste. 
- Accent color: Dark red (use for highlights, active states, glows)
...
- 4 metric cards in a grid: each with colored top accent bar (3px), icon, label, value, and trend indicator
  Cards: [Positive Revenue] progress, Active Projects count, Tasks Today count, Days to Goal countdown
